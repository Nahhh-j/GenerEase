...
def issue_access(sub: str):
    data = {"sub": sub, "exp": datetime.datetime.utcnow() + datetime.timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)}
    access_token = jwt.encode(data, SECRET_KEY, ALGORITHM)
    return access_token

def issue_refresh(sub: str):
    data = {"sub": sub, "exp": datetime.datetime.utcnow() + datetime.timedelta(minutes=REFRESH_TOKEN_EXPIRE_MINUTES)}
